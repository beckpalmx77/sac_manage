...
if ($_POST["action"] === 'ADD') {
    if ($_POST["pgroup_name"] !== '') {
        //$pgroup_id = "X-" . sprintf('%04s', LAST_ID($dbh, "ims_pgroup", 'id') + 1);
...
## Total number of records without filtering
    $stmt = $dbh->prepare("SELECT COUNT(*) AS allcount FROM ims_pgroup ");
    $stmt->execute();
    $records = $stmt->fetch();
    $totalRecords = $records['allcount'];

## Total number of records with filtering
    $stmt = $dbh->prepare("SELECT COUNT(*) AS allcount FROM ims_pgroup WHERE 1 " . $searchQuery);
    $stmt->execute($searchArray);
    $records = $stmt->fetch();
    $totalRecordwithFilter = $records['allcount'];

## Fetch records
    $stmt = $dbh->prepare("SELECT * FROM ims_pgroup WHERE 1 " . $searchQuery
